...
do $guard$ begin
 if md5(pg_get_functiondef('public.persist_media_watch(uuid,text,jsonb,jsonb,text,bigint,uuid,boolean)'::regprocedure)) <> '187ff83ffcbed58e401355dafe1f1486' then raise exception 'Unexpected function: stop'; end if;
 if exists(select 1 from supabase_migrations.schema_migrations where version='20260925130000') then raise exception 'Migration already registered: stop'; end if;
end $guard$;
...
values('20260925130000','media_conflict_no_transaction_retry',array[
 pg_get_functiondef('public.persist_media_watch(uuid,text,jsonb,jsonb,text,bigint,uuid,boolean)'::regprocedure),
 'revoke all on function public.persist_media_watch(uuid,text,jsonb,jsonb,text,bigint,uuid,boolean) from public,anon;',
 'grant execute on function public.persist_media_watch(uuid,text,jsonb,jsonb,text,bigint,uuid,boolean) to authenticated;'
]);
commit;
select now() as applied_at, md5(pg_get_functiondef('public.persist_media_watch(uuid,text,jsonb,jsonb,text,bigint,uuid,boolean)'::regprocedure)) as function_md5,
 (select count(*) from supabase_migrations.schema_migrations where version='20260925130000') as registration_count;

...
export default defineConfig(({ mode }) => {
  ...
  const diagnosticsMiddleware = createArticleAnalysisDiagnosticsMiddleware({
    apiKey: process.env.OPENAI_API_KEY || env.OPENAI_API_KEY,
    model: process.env.OPENAI_MODEL || env.OPENAI_MODEL || 'gpt-5.6-luna',
    environment: { ...process.env, NODE_ENV: mode === 'production' ? 'production' : 'development' },
  });
  const urlWatchPlugin = {
    name: 'url-watch-prototype-api',
    configureServer(server) {
      server.middlewares.use(diagnosticsMiddleware);

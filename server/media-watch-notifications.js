...
const SAFE_CODES = new Set(['DATABASE_ERROR', 'RECIPIENT_UNVERIFIED', 'RECIPIENT_LOCALE_UNAVAILABLE',
  ...
  'EMAIL_DELIVERY_OUTCOME_UNKNOWN', 'MEDIA_DEADLINE_EXCEEDED', 'EMAIL_DELIVERY_FAILED']);
const safeCode = (error) => SAFE_CODES.has(error?.code) ? error.code : 'EMAIL_DELIVERY_FAILED';
...
export const processMediaWatchEmailNotifications = async ({ client, env = process.env, sender = sendWithResend, createClaimToken = randomUUID, deadline = Date.now() + 30000 } = {}) => {
  ...
  await mapBounded(pending || [], async ({ id }) => { const token = createClaimToken(); let claimed; let boundaryAttempted = false; let deliveredSuccessfully = false;
    ...
    } catch (failure) { failedCount += 1; if (claimed) { try { await request(() => client.rpc('fail_media_watch_email_notification', { p_notification_id: id, p_claim_token: token,
        p_error_code: boundaryAttempted && (deliveredSuccessfully || !['EMAIL_PROVIDER_ERROR','EMAIL_RATE_LIMITED'].includes(failure.code))
          ? 'EMAIL_DELIVERY_OUTCOME_UNKNOWN' : safeCode(failure) })); } catch { /* isolate rows */ } } }
  });

...
const formatDate = (value, locale) => { const date = new Date(value); return Number.isNaN(date.getTime()) ? '' : new Intl.DateTimeFormat(locale === 'fr' ? 'fr-FR' : 'en-GB', { day: 'numeric', month: 'long', year: 'numeric', timeZone: 'UTC' }).format(date); };
...
export const renderMediaWatchEmail = ({ locale, watchId, watchTitle, article, baseUrl }) => {
  ...
  const detailUrl = new URL('/watch-detail.html', baseUrl); detailUrl.searchParams.set('id', watchId);
